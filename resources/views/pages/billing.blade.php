    ...
    <div class="px-6 py-12 dark:bg-gray-950 dark:text-white shadow-sm h-screen">
        <div>
            <h1 class="text-3xl font-bold">
                <x-filament-panels::logo class="pb-4 h-80" />
            ...
            <x-filament::link href="{{ url(filament()->getCurrentPanel()->getPath()) }}" class="mt-6" >
                ...
                <div class="flex flex-col gap-4">
                    @forelse ($plans as $plan)
                        <x-filament::section
                            ...
                            :style="$currentSubscription->plan()->is($plan) ? 'background: rgba(var(--secondary-200), 0.25);' : ''"
                        >
                            <div>
                               <div>
                                   @if ($plan->isFree())
                                       <span class="text-3xl font-bold">{{ trans('filament-subscriptions::messages.view.free') }}</span>
                                   @else
                                ...
                                @foreach ($plan->features->sortBy('sort_order') as $feature)
                                    <div class="flex justifiy-start gap-2">
                                        <div>
                                            ...
                                            @else
                                                <svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" data-slot="icon" class="flex-shrink-0 w-5 h-5 text-gray-400">
